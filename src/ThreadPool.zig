//! This is pulled from std.Thread.Pool. We've modified it to allow the spawned function to return
//! an error. This helps with cleanup. All errors are logged
...
pub fn spawn(pool: *Pool, comptime func: anytype, args: anytype) !void {
    ...
    const Closure = struct {
        ...
        fn runFn(runnable: *Runnable, _: ?usize) void {
            const run_node: *RunQueue.Node = @fieldParentPtr("data", runnable);
            const closure: *@This() = @alignCast(@fieldParentPtr("run_node", run_node));
            @call(.auto, func, closure.arguments) catch {};
